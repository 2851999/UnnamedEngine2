...
void main() {
	...
	//Check if material is reflective and have not already reached maximum recursion
	if (mat.shininess > 0.1 && rayPayload.depth < 10) {
		float tMin = 0.001;
		float tMax = 10000.0;
	...
	rayPayload.hitValue += vec3(lightIntensity * attenuation * (diffuse + specular)) * rayPayload.attenuation;
