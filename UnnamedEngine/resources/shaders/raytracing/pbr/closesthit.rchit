...
layout(set = 1, binding = 9) uniform samplerCube ue_irradianceMap;
layout(set = 1, binding = 10) uniform samplerCube ue_prefilterMap;
layout(set = 1, binding = 11) uniform sampler2D ue_brdfLUT;
...
vec3 ueGetLightingPBR(vec3 normal, vec3 fragPos, vec3 albedo, float metalness, float roughness, float ao) {
    ...
    if (ue_lightAmbient.r > 0.0) {
		...
		// if (metalness > 0.05 && rayPayload.done != 2) {
		// 	rayPayload.done = 2;
		...
		// 	reflection = rayPayload.hitValue;
		// }
		...
		//vec3 prefilteredColor = reflection;
