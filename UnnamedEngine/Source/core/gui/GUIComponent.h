...
public:
	...
	GUIComponentRenderer(float width, float height) :
		GameObject2D(new Mesh(MeshBuilder::createQuad(width, height, MESH_DATA_FLAGS)), "Material", width, height) { setup(); }
	GUIComponentRenderer(float width, float height, std::vector<Colour> colours) :
		GameObject2D(new Mesh(MeshBuilder::createQuad(width, height, MESH_DATA_FLAGS)), "Material", width, height), colours(colours) { setup(); }
	GUIComponentRenderer(float width, float height, std::vector<Texture*> textures) :
		GameObject2D(new Mesh(MeshBuilder::createQuad(width, height, textures.at(0), MESH_DATA_FLAGS)), "Material", width, height), textures(textures) { setup(); }
	GUIComponentRenderer(float width, float height, std::vector<Colour> colours, std::vector<Texture*> textures) :
		GameObject2D(new Mesh(MeshBuilder::createQuad(width, height, textures.at(0), MESH_DATA_FLAGS)), "Material", width, height), colours(colours), textures(textures) { setup(); }
