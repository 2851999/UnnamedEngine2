...
CDLODHeightMap::CDLODHeightMap(std::string path) {
	...
	this->data = Texture::loadTexture(path, numComponents, w, h, false);
	//Setup this height map
	setup(data, numComponents, w, h);
}

CDLODHeightMap::CDLODHeightMap(unsigned char* data, int numComponents, int width, int height) {
	this->data = data;
	//Setup this height map
	setup(data, numComponents, width, height);
}
...
void CDLODHeightMap::setup(unsigned char* data, int numComponents, int width, int height) {
	...
	if (width == height && width != 0 && (width & (width - 1)) == 0) {
		...
		TextureParameters p;
		p.setFilter(TextureParameters::Filter::LINEAR);
		p.setAddressMode(TextureParameters::AddressMode::REPEAT);
