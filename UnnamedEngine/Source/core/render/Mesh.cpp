...
void MeshRenderData::setup(MeshData* data, std::vector<Material*>& materials, VBOUsage vboUsage, bool updatable) {
	//The shader used for the setup
	Shader* shader = setupShader->getShader();

	//Setup positions
	if (data->hasPositions() && data->separatePositions()) {
		vboPositions = new VBO<float>(GL_ARRAY_BUFFER, data->getPositions().size() * sizeof(data->getPositions()[0]), data->getPositions(), vboUsage, false, updatable);
	...
	if (data->hasColours() && data->separateColours()) {
		vboColours = new VBO<float>(GL_ARRAY_BUFFER, data->getColours().size() * sizeof(data->getColours()[0]), data->getColours(), vboUsage, false, updatable);
	...
	if (data->hasTextureCoords() && data->separateTextureCoords()) {
		vboTextureCoords = new VBO<float>(GL_ARRAY_BUFFER, data->getTextureCoords().size() * sizeof(data->getTextureCoords()[0]), data->getTextureCoords(), vboUsage, false, updatable);
	...
	if (data->hasNormals() && data->separateNormals()) {
		vboNormals = new VBO<float>(GL_ARRAY_BUFFER, data->getNormals().size() * sizeof(data->getNormals()[0]), data->getNormals(), vboUsage, false, updatable);
	...
	if (data->hasTangents() && data->separateTangents()) {
		vboTangents = new VBO<float>(GL_ARRAY_BUFFER, data->getTangents().size() * sizeof(data->getTangents()[0]), data->getTangents(), vboUsage, false, updatable);
	...
	if (data->hasBitangents() && data->separateBitangents()) {
		vboBitangents = new VBO<float>(GL_ARRAY_BUFFER, data->getBitangents().size() * sizeof(data->getBitangents()[0]), data->getBitangents(), vboUsage, false, updatable);
	...
	if (data->hasOthers()) {
		...
		vboOthers = new VBO<float>(GL_ARRAY_BUFFER, data->getOthers().size() * sizeof(data->getOthers()[0]), data->getOthers(), vboUsage, false, updatable);
	...
	//Setup bones
	if (data->hasBones()) {
		vboBoneIDs = new VBO<unsigned int>(GL_ARRAY_BUFFER, data->getBoneIDs().size() * sizeof(data->getBoneIDs()[0]), data->getBoneIDs(), VBOUsage::STATIC, false, updatable);
		vboBoneIDs->addAttributeWithType(GL_INT, ShaderInterface::ATTRIBUTE_LOCATION_BONE_IDS, 4);
		renderData->addVBO(vboBoneIDs);

		vboBoneWeights = new VBO<float>(GL_ARRAY_BUFFER, data->getBoneWeights().size() * sizeof(data->getBoneWeights()[0]), data->getBoneWeights(), VBOUsage::STATIC, false, updatable);
	...
	if (data->hasIndices()) {
		vboIndices = new VBO<unsigned int>(GL_ELEMENT_ARRAY_BUFFER, data->getNumIndices() * sizeof(data->getIndices()[0]), data->getIndices(), vboUsage, false, updatable);
...
void Mesh::setup(RenderShader* renderShader, VBOUsage vboUsage, bool updatable) {
	this->renderData = new MeshRenderData(this->data, renderShader);
	this->renderData->setup(data, materials, vboUsage, updatable);
}
...
MeshData* MeshBuilder::createQuad(Vector2f v1, Vector2f v2, Vector2f v3, Vector2f v4, Texture* texture, MeshData::Flag flags) {
	MeshData* data = new MeshData(2, flags);
